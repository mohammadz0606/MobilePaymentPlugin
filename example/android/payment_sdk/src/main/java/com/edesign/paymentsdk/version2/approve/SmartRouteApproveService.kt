...
import com.edesign.paymentsdk.Approve.ApproveRequest
 ...
 */
class SmartRouteApproveService
/*Context used as a parameter*/(private val context: Context) {
    private var authenticationToken: String? = ""
    private var req = ApproveRequest()
    private var response: ApproveResponse? = null
    private var utility: Utility? = null
    private var param: ApproveCallback? = null
     ...
     */
    fun process(requestSTS: ApproveRequest, param1: ApproveCallback) {
        ...
        if (utility!!.isNetworkAvailable) {
            ...
            if (readGenericFile() != null &&
                ...
                req.getGenericKeyValue().containsKey(Parameters.APPROVE_END_POINT) &&
                !req.getGenericKeyValue()[Parameters.APPROVE_END_POINT]!!.isNullOrEmpty() &&
             ...
             * for secure hash generation check detail inside method*/
                ...
                if (authenticationToken != null && !authenticationToken!!.isEmpty()) {
                    try {
                        ...
                        var call = apiInterface.mPaymentApi(
                            req.getGenericKeyValue()[Parameters.APPROVE_END_POINT]!!,
                            req.getParameters()
                        )
                        call.enqueue(object : Callback<okhttp3.ResponseBody> {
                            ...
                            override fun onFailure(call: Call<okhttp3.ResponseBody>, t: Throwable) {
                                //t.printStackTrace()
                                /*In case of Abnormal Error we return execCode is 500 */
                                if (!response!!.response.containsKey(ErrorCodesMessage.kErrorCode)) {
                                    response!!.addResponse(
                                        ErrorCodesMessage.kErrorCode,
                                        ErrorCodesMessage.kAbnormalErrorCode
                                    )
                        ...
                        /*In case of Abnormal Error we return code is 500 */
                        if (!response!!.response.containsKey(ErrorCodesMessage.kErrorCode)) {
                            response!!.addResponse(
                                ErrorCodesMessage.kErrorCode,
                                ErrorCodesMessage.kAbnormalErrorCode
                            ...
                            response!!.addResponse(
                                ErrorCodesMessage.kErrorDescription, e.printStackTrace().toString())
                 ...
                 the authentication token from merchant side than return user code is 1000*/
                } else {
                    if (!response!!.response.containsKey(ErrorCodesMessage.kErrorCode)) {
                ...
                /*We return code if Missing Configuration in resource files*/
            } else {
                if (!response!!.response.containsKey(ErrorCodesMessage.kErrorCode)) {
        ...
        * key for url name and value for url link*/
        try {
            ...
            try {
                ...
                response!!.addResponse(
                    ErrorCodesMessage.kErrorDescription, ErrorCodesMessage.kGenericFileMessage
                )
