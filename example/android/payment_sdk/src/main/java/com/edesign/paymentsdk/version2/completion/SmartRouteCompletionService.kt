 ...
 */
class SmartRouteCompletionService
/*Context used as a parameter*/(private val context: Context) {
    private var authenticationToken: String? = ""
    private var req = CompletionRequest()
    private var response: CompletionResponse? = null
    private var utility: Utility? = null
    private var param: CompletionCallback? = null
     ...
     */
    fun process(requestSTS: CompletionRequest, param1: CompletionCallback) {
        ...
        if (utility!!.isNetworkAvailable) {
            ...
            if (readGenericFile() != null &&
                ...
                req.getGenericKeyValue().containsKey(Parameters.COMPLETION_END_POINT) &&
                !req.getGenericKeyValue()[Parameters.COMPLETION_END_POINT]!!.isNullOrEmpty() &&
             ...
             * for secure hash generation check detail inside method*/
                ...
                if (authenticationToken != null && !authenticationToken!!.isEmpty()) {
                    try {
                        ...
                        var call = apiInterface.mPaymentApi(
                            req.getGenericKeyValue()[Parameters.COMPLETION_END_POINT]!!,
                            req.getParameters()
                        )
                        call.enqueue(object : Callback<okhttp3.ResponseBody> {
                            ...
                            override fun onFailure(call: Call<okhttp3.ResponseBody>, t: Throwable) {
                                ...
                                if (!response!!.response.containsKey(ErrorCodesMessage.kErrorCode)) {
                                    response!!.addResponse(ErrorCodesMessage.kErrorCode, ErrorCodesMessage.kAbnormalErrorCode)
                                    response!!.addResponse(ErrorCodesMessage.kErrorDescription, t.printStackTrace().toString())
                        ...
                        if (!response!!.response.containsKey(kErrorCode)) {
                            ...
                            response!!.addResponse(ErrorCodesMessage.kErrorDescription, e.printStackTrace().toString())
                 ...
                 the authentication token from merchant side */
                    ...
                    if (!response!!.response.containsKey(kErrorCode)) {
                        response!!.addResponse(kErrorCode, ErrorCodesMessage.kSDKNotInitializedErrorCode)
                ...
                if (!response!!.response.containsKey(kErrorCode)) {
                    response!!.addResponse(kErrorCode, ErrorCodesMessage.kConfigFileErrorCode)
                    response!!.addResponse(ErrorCodesMessage.kErrorDescription, ErrorCodesMessage.kConfigFileMessage)
            ...
            if (!response!!.response.containsKey(kErrorCode)) {
                response!!.addResponse(kErrorCode, ErrorCodesMessage.kNetworkConnection)
                response!!.addResponse(ErrorCodesMessage.kErrorDescription, ErrorCodesMessage.kNetworkConnectionMessage)
        ...
        * key for url name and value for url link*/
        try {
            ...
            try {
                ...
                /*In case of Abnormal Error  */
                response!!.addResponse(kErrorCode, ErrorCodesMessage.kGenericFileError)
                response!!.addResponse(ErrorCodesMessage.kErrorDescription, ErrorCodesMessage.kGenericFileMessage)
