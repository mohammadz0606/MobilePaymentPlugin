 ...
 * * * Using this OpenPaymentRequest class merchant mobile application should be able to send a Open Payment message
 * using Smart Route interface only.
 */
class OpenPaymentRequest:Serializable {
   ...
   *To enabled or disabled Log in Smart Route Approve Payment */
    ...
    fun replaceValue(key:String, value:String){
        parameters.replace(key,value)
    }

    fun addOptional(key: String, value: Any) {
        optionalPaymentRequest.add(key,value)
    }

    fun getOptionalParam():OptionalPaymentRequest {
        return optionalPaymentRequest
    }
}