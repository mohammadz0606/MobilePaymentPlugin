...
import com.edesign.paymentsdk.Sdk.PaymentGatway.SmartApprovePayment;
 ...
 * Approve Services class handle the Approve request
 * for  smart route interface and process the app request on the bases of app request
 * and return the proper response object to the app
 */
public class ApproveService {
    private Context context;
    private String authenticationToken = "";
    private ApproveRequest req = new ApproveRequest();
    ...
    public ApproveResponse process(ApproveRequest request) {
        ...
        if (utility.isNetworkAvailable()) {
            ...
            */
            ...
            if (readGenericFile() != null && utility.readWordExcludedParameterFromSecureHash() != null && utility.readWordParameterToEncodeForSecureHash() != null && utility.readWordsFromParameterToEncode() != null && req.getGenericKeyValue().containsKey(Parameters.BYPASS_SECURE_HASH_VALIDATION) &&
                    !req.getGenericKeyValue().get(Parameters.BYPASS_SECURE_HASH_VALIDATION).isEmpty() && req.getGenericKeyValue().get(Parameters.APPROVE_URL) != null &&
                    req.getGenericKeyValue().containsKey(Parameters.APPROVE_URL) && !req.getGenericKeyValue().get(Parameters.APPROVE_URL).isEmpty()) {
                try {
                ...
                if (authenticationToken != null && !authenticationToken.isEmpty()) {
                    ...
                    try {
                        ...
                        if (request.getPaymentType() != null && request.getPaymentType().equals(Parameters.SMART_ROUTE_VALUE)) {
                            if (SmartApprovePayment.approveResSm != null) {
                                SmartApprovePayment.approveResSm.clear();
                            }
                            if (SmartApprovePayment.timeOutList != null) {
                                SmartApprovePayment.timeOutList.clear();
                            }
                        ...
                        try {
                            if (request.getPaymentType().equals(Parameters.SMART_ROUTE_VALUE)) {
                                if (SmartApprovePayment.timeOutList != null && SmartApprovePayment.timeOutList.size() != Parameters.NUMERIC_ZERO) {
                                    response.addResponse(Parameters.EXEC_CODE, Parameters.EXEC_CODE_TWO);
                                    response.addResponse(Parameters.RESPONSE_HASH_MATCH, Parameters.MATCHED_VALUE);
                                } else if (SmartApprovePayment.approveResSm != null && !SmartApprovePayment.approveResSm.containsKey("") && SmartApprovePayment.approveResSm.size() != Parameters.NUMERIC_ZERO) {
                                    response.addResponse(Parameters.EXEC_CODE, Parameters.EXEC_CODE_ZERO);
                                    for (Map.Entry<String, String> entry : SmartApprovePayment.approveResSm.entrySet()) {
    ...
    private void secureHashValidation() {
        ...
        * if missing then status code and status description return to user  */
        ...
        try {
            if (req.getPaymentType().equals(Parameters.SMART_ROUTE_VALUE)) {
                if (req.getGenericKeyValue().get(Parameters.BYPASS_SECURE_HASH_VALIDATION).equals(Parameters.N_VALUE)) {
                    if (!SmartApprovePayment.approveResSm.containsKey("") && SmartApprovePayment.approveResSm.get(Parameters.SMART_ROUTE_RESPONSE_STATUS).equals(Parameters.SMART_ROUTE_SUCCESS)) {

                         /*Now that we have the map, order it to generate secure hash and compare it with the received one*/

                        generatedSecureHash = utility.secureHashForResponse(authenticationToken, SmartApprovePayment.approveResSm);
                        System.out.println("Generated secureHash: " + generatedSecureHash);

                        /*get the received secure hash from res map*/

                        receivedSecureHash = SmartApprovePayment.approveResSm.get(Parameters.RESPONSE_SECURE_HASH);
                        System.out.println("Received secureHash: " + receivedSecureHash);
                        System.out.println("Message ID: " + SmartApprovePayment.approveResSm.get("Response.MessageID") + " - transaction ID: " + SmartApprovePayment.approveResSm.get("Response.TransactionID") + ", Generated secureHash: " + generatedSecureHash);
                        System.out.println("Message ID: " + SmartApprovePayment.approveResSm.get("Response.MessageID") + " - transaction ID: " + SmartApprovePayment.approveResSm.get("Response.TransactionID") + ", Received secureHash: " + receivedSecureHash);
                        if (!receivedSecureHash.equals(generatedSecureHash)) {
                            ...
                            String PaymentStatus = SmartApprovePayment.approveResSm.get(Parameters.SMART_ROUTE_RESPONSE_STATUS);
                            System.out.println("Approve status: " + PaymentStatus);
                            response.addResponse(Parameters.RESPONSE_HASH_MATCH, Parameters.MATCHED_VALUE);
                        }
                    } else if (SmartApprovePayment.approveResSm != null && !SmartApprovePayment.approveResSm.containsKey("") && SmartApprovePayment.approveResSm.size() != Parameters.NUMERIC_ZERO) {
                        if (!SmartApprovePayment.approveResSm.containsKey(Parameters.RESPONSE_SECURE_HASH)) {
                            response.addResponse(Parameters.RESPONSE_HASH_MATCH, Parameters.MATCHED_VALUE);
                        } else {
    ...
    //Smart route Approve sendRequest to payment Gateway
    private void sendDataToServer() {

        /*Get ApproveURL from resource file */

        String url = req.getGenericKeyValue().get(Parameters.APPROVE_URL);
        System.out.println("ApproveURL: " + url);
        try {
            ...
            /*Here we send request to the SmartApprovePayment gateway */

            SmartApprovePayment.sendRequest(context, authenticationToken, requestQuery, url);
        } catch (Exception e) {
